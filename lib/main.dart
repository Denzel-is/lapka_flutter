...
class ProductDetailsDialog extends StatelessWidget {
  final Product product;
  final UserModel currentUser;
  const ProductDetailsDialog({super.key, required this.product, required this.currentUser});
      ...
      content: SingleChildScrollView(
        child: Column(
          ...
          children: [
            ...
            const Divider(),
            const SizedBox(height: 6),
            const Text('Товары в заказе:', style: TextStyle(fontWeight: FontWeight.bold)),
            const SizedBox(height: 6),
            Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: order.items.map((item) {
                return Padding(
                  ...
                  child: Text('- ${item.name} (${item.price.toStringAsFixed(0)} тг)'),
                );
              }).toList(),
            ),
          ],
